...
//		WriteBuf[i] = 0;
//	}
//	Status = XSpi_WriteReg(SpiInstance->BaseAddr, reg);

	Status = XSpi_SetSlaveSelect(&SpiInstance, 1);
	if(Status) {
		xil_printf("SS Err: %d", Status);
	}

	Status = XSpi_Transfer(&SpiInstance, WriteBuf, NULL, 2);
	if(Status) {
		xil_printf("WR Error code %d", Status);
	}
...
//	xil_printf("single wr: %x at reg: %d\n", val, reg >> 3 );
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	//return (data + nbytes);
	BYTE ReadBuf[nbytes + 1];
	BYTE WriteBuf[nbytes + 1];
	WriteBuf[0] = reg + 0x2;
...
//	xil_printf("single rd: %x at reg: %d\n", ReadBuf[1], reg >> 3 );
